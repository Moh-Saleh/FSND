...
def venues():
    ...
    #     }]
    # }]
    locations = Venue.query.with_entities(func.count(Venue.id), Venue.city, Venue.state).group_by(Venue.city,
                                                                                                  Venue.state).all()
    data = []
        ...
        for venue in venues_location:
            venue_data.append({
                ...
                "num_upcoming_shows": len(
                    db.session.query(Show).filter(Show.venue_id == 1).filter(Show.start_date > datetime.now()).all())
            })
        data.append({
            "city": location.city,
...
def search_venues():
    ...
    results = db.session.query(Venue).filter(Venue.name.ilike(f'%{term}%')).all()
    data = []
...
def show_venue(venue_id):
    ...
    upcomings = db.session.query(Show).join(Artist).filter(Show.venue_id == venue_id).filter(
        Show.start_date > datetime.now()).all()
    upcoming_shows = []

    pasts = db.session.query(Show).join(Artist).filter(Show.venue_id == venue_id).filter(
        Show.start_date < datetime.now()).all()
    past_shows = []
    ...
    for show in pasts:
        past_shows.append({
            ...
            "start_date": show.start_date.strftime('%Y-%m-%d %H:%M:%S')
        })

    for show in upcomings:
...
def search_artists():
    ...
    # }
    term = request.form.get('term', '')
    results = db.session.query(Artist).filter(Artist.name.ilike(f'%{term}%')).all()
    data = []
    for result in results:
        data.append({
            "id": result.id,
...
def show_artist(artist_id):
    ...
    pasts = db.session.query(Show).join(Venue).filter(Show.artist_id == artist_id).filter(
        Show.start_date > datetime.now()).all()
    past_shows = []
    for show in pasts:
        past_shows.append({
    ...
    upcomings = db.session.query(Show).join(Venue).filter(Show.artist_id == artist_id).filter(
        Show.start_date > datetime.now()).all()
    upcoming_shows = []
    for show in upcomings:
        upcoming_shows.append({
